...
public sealed partial class DapSymbolTests
{
    ...
    {
        string program = SymbolFixtures.GetIncompleteAppHostPath(missingAssembly);
        string? directory = Path.GetDirectoryName(program);
        Assert.IsNotNull(directory);
        Assert.IsTrue(File.Exists(program));
        Assert.AreEqual(!missingAssembly, File.Exists(Path.ChangeExtension(program, ".dll")));
        Assert.IsFalse(File.Exists(Path.ChangeExtension(program, ".runtimeconfig.json")));
