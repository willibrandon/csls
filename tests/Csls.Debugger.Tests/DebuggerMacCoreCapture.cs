...
internal static class DebuggerMacCoreCapture
{
    ...
    internal static async Task CaptureAsync(int processId, string path, Action<string> progress,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await RunAsync("/usr/bin/codesign",
                ["--force", "--sign", "-", "--entitlements", entitlements, collector], progress, cancellationToken)
        ...
        CancellationToken cancellationToken)
    {
        ...
        (int exitCode, string output, string error) = await DebuggerTestProcess.RunAsync(start, cancellationToken, progress)
            .ConfigureAwait(false);
