...
public sealed class McpProcessSessionTests
{
    ...
    /// <summary>
    /// Releases the exact process handle when a disposed diagnostic file rejects server output during disconnect.
    /// </summary>
    [TestMethod]
    [DataRow(false)]
    [DataRow(true)]
    [Timeout(30000, CooperativeCancellation = true)]
    public async Task FailedDiagnosticDestinationReleasesProcessHandle(bool explicitDisconnect)
    {
        string repository = EditorToolResolver.FindRepositoryRoot();
        DirectoryInfo directory = Directory.CreateTempSubdirectory("csls-mcp-closed-diagnostics-");
        try
        {
            using var output = new StreamWriter(Path.Join(directory.FullName, "server.log"));
            ...
            McpProcessSession session = await McpProcessSession.StartAsync(
                ...
                TestContext.CancellationToken,
                diagnosticOutput: output).ConfigureAwait(false);
            // Retain a cleanup owner even when the session's lifetime assertions fail.
            using Process launcher = session.LauncherProcess;
            SafeProcessHandle handle = launcher.SafeHandle;
            await using (session.ConfigureAwait(false))
            {
                ...
                if (explicitDisconnect)
                {
                    IOException disconnect = await Assert.ThrowsExactlyAsync<IOException>(async () =>
                        await session.DisconnectAsync(TimeSpan.FromSeconds(10), TestContext.CancellationToken)
                            .ConfigureAwait(false)).ConfigureAwait(false);
                    Assert.IsInstanceOfType<ObjectDisposedException>(disconnect.InnerException);
                    Assert.IsTrue(launcher.HasExited);
                    Assert.IsFalse(handle.IsClosed, "Disconnect leaves final process ownership with the session.");
                }
                ...
                Assert.AreEqual("Forwarding command progress failed.", failure.Message);
                Assert.IsInstanceOfType<ObjectDisposedException>(failure.InnerException);
                Assert.IsTrue(handle.IsClosed, "A failed diagnostic destination must not retain the launcher's native handle.");
                await session.DisposeAsync().ConfigureAwait(false);
            }
        }
        finally
