...
responsive during inspection, source downloads, and evaluation. Canceling queued
work removes it before execution; canceling active work propagates to its engine
or download operation. Completed results retain their responses, and request
cancellation never truncates a DAP frame already being written. Pending work is
limited to 64 requests and 16 MiB of total wire payload, with an explicit error for
overflow. During target evaluation, cancellation and deadline expiry use
