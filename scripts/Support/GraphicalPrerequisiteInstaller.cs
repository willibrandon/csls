...
/// <summary>
/// Provisions graphical editor dependencies using the host's package metadata.
/// </summary>
internal static class GraphicalPrerequisiteInstaller
{
    /// <summary>
    /// Executes a validated prerequisite request and reports provisioning failures.
    /// </summary>
    internal static async Task<int> RunAsync(GraphicalPrerequisiteOptions options)
    {
        try
        {
            ...
            if (options.WritePortableCacheKey)
            {
                await WritePortableCacheKeyAsync(options.InstallClipboard, options.InstallVulkan)
                    .ConfigureAwait(false);
                return 0;
            }

            if (options.RefreshPackageIndex)
            {
                await RunPrivilegedAsync("apt-get", ["update", "--error-on=any"])
                    .ConfigureAwait(false);
            }

            if (options.WritePackageCacheKey || options.PackageCachePath is not null)
            {
                IReadOnlyList<string> packages = await ResolveWebBrowserPackagesAsync(options.WebBrowsers)
                    .ConfigureAwait(false);
                if (options.WritePackageCacheKey)
                {
                    await AptPackageCache.WriteCacheKeyAsync(packages).ConfigureAwait(false);
                }
                else if (options.PackageCachePath is not null)
                {
                    await AptPackageCache.ProvisionAsync(packages, Path.GetFullPath(options.PackageCachePath), options.DownloadOnly)
                        .ConfigureAwait(false);
                }
            ...
            if (!options.WebOnly)
            {
                IReadOnlyList<string> graphicalPackages = await ResolveGraphicalPackagesAsync(
                    options.InstallClipboard,
                    options.InstallVulkan).ConfigureAwait(false);
                if (options.PortablePackages)
                {
                    ...
                    (portableXclipPath, string? vulkanLibraryPath, string? vulkanManifestPath) =
                        await ProvisionPortablePackagesAsync(options.InstallClipboard, options.InstallVulkan)
                            .ConfigureAwait(false);
            ...
            if (options.InstallTreeSitter && !options.WebOnly)
            {
            ...
            if (options.WebBrowsers.Count > 0)
            {
                await InstallPackagesAsync(
                    await ResolveWebBrowserPackagesAsync(options.WebBrowsers).ConfigureAwait(false))
                    .ConfigureAwait(false);
            }
    ...
    private static async Task<IReadOnlyList<string>> ResolveGraphicalPackagesAsync(
    ...
    private static async Task<IReadOnlyList<string>> ResolveWebBrowserPackagesAsync(
    ...
    private static async Task WritePortableCacheKeyAsync(bool installClipboard, bool installVulkan)
    {
    ...
    private static async Task<string> ResolveCandidateVersionAsync(string packageName)
    {
    ...
    private static async Task<(string? XclipPath, string? VulkanLibraryPath, string? VulkanManifestPath)>
    ...
    private static string FindRequiredFile(string rootPath, string fileName) =>
        Directory.EnumerateFiles(rootPath, fileName, SearchOption.AllDirectories).Single();

    private static async Task ExportPortableEnvironmentAsync(
        string? xclipPath,
    ...
    private static async Task<string> ReadRequiredOutputAsync(
    ...
    private static async Task InstallPackagesAsync(IReadOnlyList<string> packages)
    {
    ...
    private static async Task<bool> PackageExistsAsync(string packageName)
    {
    ...
    private static async Task<bool> ArePackagesInstalledAsync(IReadOnlyList<string> packageNames)
    {
        (int exitCode, string output, _) = await RunAsync(
            "dpkg-query",
            [
    ...
    private static async Task<string> SelectMatchingPackageAsync(string packageNamePattern)
    {
