...
for each loaded module. A logical breakpoint creates fresh runtime bindings after
module unload/reload, collectible assembly churn, or target restart.
...
`N` is a positive decimal integer. Conditional breakpoints count hits whose condition
evaluates to true. Counts span all runtime bindings for that logical breakpoint and
reset when the client replaces it. Each requested breakpoint receives its own binding result.

## Logpoints

A source breakpoint with a non-empty `logMessage` logs and continues execution. Expressions
...
hit-count behavior with source breakpoints. Binding diagnostics identify ambiguous
or unresolved method names.
...
and available stack details, preserving the exception stage.
...
instruction boundary. CoreCLR binding failures mark the affected item as unverified
with the runtime diagnostic. Instruction breakpoints rebind when the
...
enters that exact call, including repeated calls to the same method on one line.
Eligible calls have a managed implementation in the same module and a matching Portable PDB.

## Go to Line

`gotoTargets` returns destinations approved by CoreCLR in the active managed method.
Approval requires `ICorDebugILFrame.CanSetIP` to return `S_OK`. The adapter repeats
that check immediately before `goto`. A successful response precedes the resulting
stopped event. Target identifiers expire when the stop generation changes.
