...
| `debug_session_start` | Destructive | None | `program` required, `workingDirectory` required, `arguments`, `environment`, `environmentFilePath`, `runtimeHostPath`, `sourceFileMap`, `initialSourcePath`, `initialLine`, `suppressJitOptimizations`, `enableHotReload`, `justMyCode`, `enableStepFiltering`, `stopAtEntry`, `requireExactSource`, `expressionEvaluationOptions` | Launch one managed .NET target in an isolated debugger worker and return its explicit debugSession identifier. |
