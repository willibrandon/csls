...
| `debug_session_attach` | Destructive | None | `processId` required, `pause`, `sourceFileMap`, `requireExactSource` | Attach an isolated debugger worker to one explicit managed process and return its debugSession identifier. |
...
| `debug_session_start` | Destructive | None | `program` required, `workingDirectory` required, `arguments`, `environment`, `environmentFilePath`, `runtimeHostPath`, `sourceFileMap`, `initialSourcePath`, `initialLine`, `suppressJitOptimizations`, `enableHotReload`, `justMyCode`, `enableStepFiltering`, `stopAtEntry`, `requireExactSource` | Launch one managed .NET target in an isolated debugger worker and return its explicit debugSession identifier. |
