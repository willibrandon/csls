...
Management. Projects reference those centrally versioned packages. CI restores
against the central versions and tests the resulting tools on supported runtimes.
...
and file access. Product code uses .NET framework libraries, approved Microsoft
packages, and Hex1b for terminal UI.

Roslyn analyzer, Source Link, MSBuild task, and runtime host packages used only while
building are private build assets.
...
Roslyn and MSBuild run in the managed workers.

## Development-only packages

MSTest and Microsoft Testing Platform run the test projects. BenchmarkDotNet is used
by the microbenchmark project. Documentation, provisioning, and repository
verification have separate development dependencies, reviewed through their
...
requirements for old-style .NET Framework projects. Install the compatible build
host in the environment that loads those projects.
